...
There are two ways you can use Kaltura media platform from Moodle. The first one is to use the Kaltura Moodle plugin. In this case you should install the [forked version of the Kaltura plugin](https://github.com/estevebadia/kaltura_moodle_plugin). This version allows us to modify the calls to the Kaltura service by changing the user id, as well as other minor improvements. It is not possible to change the sent user ids when using the original Kaltura Moodle plugin.
